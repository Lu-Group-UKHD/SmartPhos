...
readExperimentDIA <- function(fileTable, localProbCut = 0.75, annotation_col = c(), normalizeByProtein=FALSE) {
    ...
    #get sample name without pre- or su-ffix
...
#' Normalise phosphoproteomic measurement by full proteome measurement
#'
#' This function calculates the ratio of raw phophoproteomic intensity by the raw full proteomic intensity. The "Phosphoproteome" assay
#' in the MultiAssayExperiment will be replaced by the ratio.
#'
#' @param mae The MultiAsssayExperiment object generated by readExperimentDIA function.
#' @param replace Whether replace the original phosphoproteome assay with the normalized one. If replace = FALSE, a new assay, phosphoRatio, will be created.
#' @return This function returns a multi-assay experiment that contains the phosphoproteome and proteome data.
...
normByFullProteome <- function(mae, replace = TRUE) {
    ...
    #find the fp (non-enriched) counterparts of the pp (enriched) samples
    ...
    #deal with the situation that not every PP sample has an FP counterpart
