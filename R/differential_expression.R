...
#' @param assay A \code{character} string specifying the assay to use for the
#' analysis.
#' @param method A \code{character} string specifying the method to use for
#' differential expression analysis ("limma" or "ProDA"). Default is "limma".
#' @param condition A \code{character} string specifying the condition column in
#' colData(se). Default is \code{NULL}.
#' @param reference A \code{character} string or vector specifying the reference
#' group.
#' @param target A \code{character} string or vector specifying the target
#' group.
#' @param refTime A \code{character} string or vector specifying the reference
#' time points. Default is \code{NULL}.
#' @param targetTime A \code{character} string or vector specifying the target
#' time points. Default is \code{NULL}.
#' @param pairedTtest A \code{logical} value specifying to perform paired t-test
...
performDifferentialExp <- function(se, assay, method = "limma",
  ...
  if (!is.null(condition)) {
    if (!is.null(refTime)) {
      referenceID <- se[, se[[condition]] %in% reference &
                            se$timepoint %in% refTime]$sample
      targetID <- se[, se[[condition]] %in% target &
                         se$timepoint %in% targetTime]$sample
    } else {
  ...
  seSub$comparison <- ifelse(seSub$sample %in% referenceID,
                             "reference", "target")
  seSub$comparison <- factor(seSub$comparison, levels =
                                 c("reference", "target"))
...
#'
#' @description
#' \code{plotVolcano} generates a volcano plot to visualize differential
#' expression results.
#'
#' @param tableDE A \code{data frame} containing differential expression results
#' with columns 'ID', 'log2FC', 'pvalue', and 'Gene'.
#' @param pFilter A \code{numeric} value specifying the p-value threshold for
#' significance. Default is 0.05.
#' @param fcFilter A \code{numeric} value specifying the log2 fold-change
#' threshold for significance. Default is 0.5.
#'
#' @return A \code{ggplot2} object representing the volcano plot.
#'
#' @details
#' This function creates a volcano plot where differentially expressed genes are
#' categorized as 'Up', 'Down', or 'Not Sig' based on the provided p-value and
#' log2 fold-change thresholds. Points on the plot are color-coded to indicate
...
plotVolcano <- function(tableDE, pFilter = 0.05, fcFilter = 0.5) {
  ...
  v <- ggplot(dataVolcano, aes(x = log2FC, y = -log10(pvalue), label = Gene,
                               customdata = ID)) +
    ...
    geom_hline(yintercept = -log10(as.numeric(pFilter)), color = "darkgrey",
               linetype = "dashed") +
    annotate(x = 5.0, y = -log10(as.numeric(pFilter)) - 0.1,
             label = paste("P-value = ", as.numeric(pFilter)),
             geom = "text", size = 3, color = "darkgrey") +
    geom_hline(yintercept = -log10(0.25), color = "darkgrey",
               linetype = "dashed") +
...
#'
#' @description
#' \code{intensityBoxPlot} creates a boxplot for the Intensity data of a given
...
#' @param symbol \code{Character}. The symbol or name of the gene or feature
#' to use as the plot title.
#'
#' @return A \code{ggplot2} object representing the boxplot of the intensity
#' data.
#'
#' @details
#' This function generates a boxplot for the intensity data of a specified gene
#' or feature from a \code{SummarizedExperiment} (SE) object. The plot shows the
#' distribution of normalized intensities across different groups specified in
#' the \code{comparison} column of the SE object.
#'
#' The function can handle both grouped data and repeated measures:
#' - If the SE object does not contain a \code{subjectID} column, the function
#' plots a standard boxplot grouped by the \code{comparison} column.
#' - If the SE object contains a \code{subjectID} column, the function adds
#' lines connecting the points for each subject across the groups, providing a
#' visual indication of subject-specific changes.
#'
#' The \code{boxplot} is customized with various aesthetic elements, such as box
