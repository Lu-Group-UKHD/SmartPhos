...
create_test_mae <- function(adjusted = FALSE) {
  ...
  sample_data <- data.frame(sample = colnames(assay_data),
                            sampleType = rep(c("FullProteome", "Phospho"), each = 5),
                            sampleName = c("s1","s2","s3","s4","s5","s1","s2","s3","s4","s5"),
                            group = rep(c("A", "B"), each = 5))
  rownames(sample_data) <- colnames(assay_data)
  ...
  if (adjusted) {
    ...
    ppe <- SummarizedExperiment(assays = list(Intensity = assay_data, Intensity_adjusted = assay_data_adjusted), colData = sample_data, rowData = gene_data)
...
test_that("plotLogRatio works with normalization", {
  ...
  performCombinedNormalization <- function(maeData) {
    combined_data <- rbind(assay(maeData[["Proteome"]]), assay(maeData[["Phosphoproteome"]]))
...
test_that("runPhosphoAdjustment returns a MultiAssayExperiment object", {
  ...
  adjusted_mae <- runPhosphoAdjustment(mae, normalization = FALSE, minOverlap = 3, completeness = 0.5, ncore = 1)
...
test_that("runPhosphoAdjustment handles minOverlap parameter correctly", {
  ...
  adjusted_mae_min3 <- runPhosphoAdjustment(mae, normalization = FALSE, minOverlap = 3, completeness = 0.5, ncore = 1)
  samples_min3 <- colnames(assays(adjusted_mae_min3[["Phosphoproteome"]])[["Intensity_adjusted"]])

  # Adjust with minOverlap = 5
  adjusted_mae_min5 <- runPhosphoAdjustment(mae, normalization = FALSE, minOverlap = 5, completeness = 0.5, ncore = 1)
...
test_that("runPhosphoAdjustment handles completeness parameter correctly", {
  ...
  adjusted_mae_comp05 <- runPhosphoAdjustment(mae, normalization = FALSE, minOverlap = 3, completeness = 0.5, ncore = 1)
  features_comp05 <- rownames(assays(adjusted_mae_comp05[["Phosphoproteome"]])[["Intensity_adjusted"]])

  # Adjust with completeness = 1.0
  adjusted_mae_comp10 <- runPhosphoAdjustment(mae, normalization = FALSE, minOverlap = 3, completeness = 1.0, ncore = 1)
...
test_that("runPhosphoAdjustment handles parallel processing with ncore > 1", {
  ...
  adjusted_mae_parallel <- runPhosphoAdjustment(mae, normalization = FALSE, minOverlap = 3, completeness = 0.5, ncore = 2)
...
# Test if plotAdjustmentResults returns all required plots when adjustment has been applied
...
# Test if plotAdjustmentResults generates ratioTrendPlot correctly when all features are present in all samples
